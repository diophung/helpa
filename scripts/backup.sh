...
cleanup() {
  rm -rf "$helpa_tmp"
  if [ "$helpa_services_stopped" = true ]; then docker compose start app worker >/dev/null; fi
}
trap cleanup EXIT
...
tar -C "$helpa_tmp" -cf - . | age -r "$AGE_RECIPIENT" -o "$BACKUP_DIR/helpa-$helpa_stamp.tar.age.partial"
