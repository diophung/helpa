...
TikTok Web OAuth stores access/refresh tokens encrypted. Direct Post and inbox initialization use server-hosted `PULL_FROM_URL`, a short-lived signed URL, explicit consent and a verified owned domain. Direct Post checks current creator privacy/interactions/duration and `TIKTOK_DIRECT_POST_ELIGIBILITY=approved`; the default is unverified. The UI offers a manual export. An accepted upload stays `needs_action`, not published. TikTok status reconciliation/refresh and broader adapter contracts continue in following slices.
...
- Best-time hints wait for actual metrics. No fabricated engagement or recommended times.
