...
5. Keep `HELPA_MODE=dry_run`. Optionally provide `META_APP_ID` and `META_APP_SECRET`. OpenAI/Anthropic keys may be added to their respective environment variables, but Phase 0 never calls either provider. The initial cap is 0; the Settings UI controls the saved cap after owner creation. Do not use a ChatGPT browser/session credential in place of an OpenAI API key.
6. In your existing Meta app, configure the exact valid OAuth redirect `https://your.actual.hostname/api/channels/facebook/callback`. Phase 0 requests `pages_show_list` for Page selection. Your existing broader grants are read from token inspection, not assumed from the fact that the app exists. See [API notes](API_NOTES.md) for pinned version and verified operations.
...
12. Record the deployed commit, verification date, Page/account used and any denied capabilities in your deployment notes. Keep dry-run enabled through the later real-traffic validation. No live publishing/reply transport is implemented in Phase 0 even if HELPA_MODE is set to live.
